...
import { Drawer, DrawerBody, DrawerOverlay, DrawerContent, DrawerCloseButton, VStack, HStack, useDisclosure, Button } from "@chakra-ui/react";
...
const Content = ({ theme, toggleSideBar }) => {
    ...
    return (
        <VStack h='90%' spacing={4} py={10} justify='space-between' fontFamily='Rubik'>
            <Link to='/' >
...
const Sidebar = ({ show, toggleSideBar, theme }) => {
    return (
        <Drawer isOpen={show} onClose={toggleSideBar} placement="left" size='full'>
            ...
            <DrawerContent>
            <DrawerCloseButton />
            <DrawerBody bgColor={theme === 'dark' ? ElementColor.primary : ElementColor.transparent}>
                <Content
                    theme={theme}
